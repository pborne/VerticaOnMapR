...
#######################################
 ...
 DROP TABLE parquet.store_sales_fact_parquet_props_sort;
 CREATE EXTERNAL TABLE parquet.store_sales_fact_parquet_props_sort (
        date_key int
       ,product_key int
       ,product_version int
       ,store_key int
 ...
 ) AS COPY FROM '/mapr/vertica/hive_parquet/store_sales_fact_parquet_props_sort/*'
 ON ANY NODE PARQUET;

 SELECT * FROM parquet.store_sales_fact_parquet_props_sort limit 5;

######################
# Compute Statistics #
######################

SELECT ANALYZE_EXTERNAL_ROW_COUNT('parquet.store_sales_fact_parquet_props_sort');
SELECT ANALYZE_STATISTICS('parquet.store_sales_fact_parquet_props_sort');
...
FROM parquet.store_sales_fact_parquet_props_sort s,
...
V8 + Stats: Time: First fetch (1000 rows): 25966.211 ms. All rows formatted: 30264.375 ms
V8 + Stats: Time: First fetch (1000 rows): 27152.620 ms. All rows formatted: 31476.562 ms
