...
WHERE ros.store_key IN
   (SELECT store_key
    FROM ros.store_dimension
    WHERE store_state = 'MA') 
AND ros.vendor_key NOT IN
   (SELECT vendor_key
...
WHERE EXISTS
  (SELECT 1
   FROM  ros.vendor_dimension
   WHERE ros.vendor_dimension.vendor_key 
   = ros.store_orders_fact.vendor_key)
...
INNER JOIN ros.call_center_dimension 
  ON (ros.online_sales_fact.call_center_key = ros.call_center_dimension.call_center_key
